...
public class users {
    ...
    public users(String name, Integer age, String email, Integer id, String job, Integer tax) {
        this.fullName = name;
        this.age = age;
        this.email = email;
        this.ID = id;
    ...
    public static List<users> getUserList() {
        ...
        try {
            ...
            for (int i = 0; i < list.getLength(); i++) {
                ...
                String jobTitle = e.getElementsByTagName("jobTitle").item(0).getTextContent();
                Integer tax = Integer.parseInt(e.getElementsByTagName("tax").item(0).getTextContent());
                System.out.println(tax);
                users users = new users(name, age, email, id, jobTitle, tax);
                user.add(users);
            }
        } catch (Exception e) {
        }
...
//this function will send data which we will get from textField(input that is related to sign up(add new user)) to XML file and store them.
    ...
    public void saveToXML() {
        try {
            ...
            Element property5 = d.createElement("jobTitle");
            Element property6 = d.createElement("tax");
            ...
            property5.appendChild(d.createTextNode(this.job));
            property6.appendChild(d.createTextNode(this.tax.toString()));

            job.appendChild(property1);
            job.appendChild(property2);
            job.appendChild(property3);
            job.appendChild(property4);
            job.appendChild(property5);
            job.appendChild(property6);
